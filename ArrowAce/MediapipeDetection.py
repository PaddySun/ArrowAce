...
# 保存数据
def save_data_to_csv(data, filename="pose_analysis_data.csv", header_written=False):
	"""保存数据到CSV文件"""
	with open(filename, mode='a', newline='') as file:
		writer = csv.writer(file)
		
		if header_written:
			writer.writerow(["FrameStamp", "TimeStamp", "State", "landmarks", "Confidence"])

		for frame_data in data:
			framestamp, timestamp, state, landmarks, confidences = frame_data
			# 假设'landmarks'是一个包含所有关节点坐标的列表，且每个关节点是[x, y]格式
			# 'confidences'是一个包含所有关节点置信度的列表
			# 这里仅示例性地写出前两个关节点和对应的置信度，实际情况可能需要遍历这些列表
			row = [
				framestamp, 
				timestamp,
				state,
				[[str(lm[0]),str(lm[1])] for lm in landmarks],  # 取前两个关节点坐标
				confidences  # 取前两个关节点的置信度
			]
			writer.writerow(row)


SAVE_THRESHOLD = 100 # 每多少帧保存一次数据
...
# 初始化数据保存
data_to_save = []  # 初始化数据列表，包含表头
header_written = True
save_count = 0  # 用于追踪已添加到data_to_save的数据条目数量
	...
	while cap.isOpened():
		...
		if results.pose_world_landmarks:
			...
			if previous_LandmarkData is not None:
				...
				drawed_image = drwa_image.draw(2)
				...
				# 将关键数据结构化并保存
				data_to_save.append(
					(N, start_time, current_state, current_LandmarkData.landmarks, current_LandmarkData.confidences),
				)
				save_count += 1
				# 达到保存阈值时执行保存操作并清空列表
				if save_count >= SAVE_THRESHOLD:
					save_data_to_csv(data_to_save, f'./data/pose_analysis_data_{current_time}.csv', header_written)
					data_to_save = []  # 重置data_to_save为只含表头的状态
